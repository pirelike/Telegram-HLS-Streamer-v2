...
pub fn merge_from_export(
    ...
    for job in &export.jobs {
        ...
        merged_jobs += tx.execute(
            "INSERT OR IGNORE INTO jobs(
                ...
                created_at, media_type, series_name, has_thumbnail, is_series, season_number, episode_number, part_number
    ...
    for segment in &export.segments {
        ...
        merged_segments += tx.execute(
            "INSERT OR IGNORE INTO segments(job_id, segment_key, file_id, bot_index, file_size, duration)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params![segment.job_id, segment.segment_key, segment.file_id, *bot_index, segment.file_size, segment.duration],
        )?;
    }
    tx.commit()?;
    Ok(MergeResult {
