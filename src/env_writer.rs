...
pub fn write_env_values(env_path: &Path, env_map: &HashMap<&str, String>) -> Result<()> {
    ...
    let tmp_name = tmp_name.replace(|c: char| c == ' ' || c == '(' || c == ')' || c == '{' || c == '}', "");
    ...
    if let Err(e) = std::fs::rename(&tmp, env_path) {
        ...
        return Err(e).with_context(|| format!("renaming {} -> {}", tmp.display(), env_path.display()));
    }
        ...
        if b == b'#' {
            ...
            if !before.ends_with('\\') {
                ...
                if before.chars().last().map_or(true, |c| c.is_whitespace()) {
                    let ws_start = before.rfind(|c: char| !c.is_whitespace())
                        .map(|pos| pos + 1)
                        .unwrap_or(0);
