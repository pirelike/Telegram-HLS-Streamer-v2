...
- **Telegram titkosítás**: `TELEGRAM_ENCRYPTION_KEY` env-only, 32 byte hex (`openssl rand -hex 32`). Ha be van állítva, az új Telegram feltöltések véletlen `.dat` dokumentumok AEAD ciphertexttel; a kulcs elvesztése után ezek nem állíthatók vissza.
